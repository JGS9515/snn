...
parser.add_argument('-rc','--recurrencia',type=str,help='¿Con capa recurrente o sin capa recurrente?')
...
recurrencia=eval(args.recurrencia)
...
#output_path=f'output/{path}/{nu1_str}/{nu2_str}/{n}/{threshold}/{decay}/{amp}/{epochs}/{reso}/{recurrencia}'
...
for train_index, test_index in tscv.split(data):
    ...
    #Crea la red.
    network, source_monitor,target_monitor = crear_red(R,T,n,threshold,decay,nu1,nu2,recurrencia,device)
    ...
    secuencias2train=convertir_data(data_train,T,cuantiles,R,device,is_train=True)
    print(f'Longitud de dataset de entrenamiento: {len(secuencias2train)}')
    secuencias2test=convertir_data(data_test,T,cuantiles,R,device)
    ...
    for e in range(max(epochs)):
        ...
        if e+1 in epochs:
            ...
            output_path=f'output/{path}/{nu1_str}/{nu2_str}/{n}/{threshold}/{decay}/{amp}/{e+1}/{reso}/{recurrencia}'
            os.makedirs(output_path, exist_ok=True)
